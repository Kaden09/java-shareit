...
public class BookingServiceImpl implements BookingService {
    ...
    @Override
    public List<BookingResponseDto> getAllByBooker(Long userId, String stateString, Integer from, Integer size) {
        ...
        switch (state) {
            case ALL:
                ...
                break;
            case CURRENT:
                bookings = bookingRepository.findAllByBookerIdAndStartBeforeAndEndAfterOrderByStartDesc(userId, now, now, pageable);
                break;
            case PAST:
            ...
            default:
                ...
                throw new ValidationException("Состояние фильтрации не поддерживается: " + stateString);
        }
    ...
    @Override
    public List<BookingResponseDto> getAllByOwner(Long userId, String stateString, Integer from, Integer size) {
        ...
        switch (state) {
            case ALL:
                ...
                break;
            case CURRENT:
                bookings = bookingRepository.findAllByItemOwnerIdAndStartBeforeAndEndAfterOrderByStartDesc(userId, now, now, pageable);
                break;
            case PAST:
                bookings = bookingRepository.findAllByItemOwnerIdAndEndBeforeOrderByStartDesc(userId, now, pageable);
                break;
            case FUTURE:
                bookings = bookingRepository.findAllByItemOwnerIdAndStartAfterOrderByStartDesc(userId, now, pageable);
                break;
            case WAITING:
    ...
    private BookingState parseState(String state) {
        ...
        try {
            ...
            throw new ValidationException("Состояние фильтрации не поддерживается: " + state);
        }
    }
}